$(document).ready(function () {
  ...
  $( "#submit_auditors" ).click(function(e) {
    ...
    if (title == '' || name == '' || uname == '' || agency == '' || position == '' || email == '' || pwd == '') {
      jQuery(".error_create").fadeIn( 100 , function() {
        
      }).fadeOut(10000);
    }else{

    var srow = $('#auditors_row');
    var form = document.querySelector('#storeAuditorsForm');
    ...
    success : function(data){
      ...
      $('#u_i_id').val(data.user_id);

      $( '#storeAuditorsForm' ).each(function(){
          this.reset();
      });
      ...
      jQuery(".success_create").fadeIn( 100 , function() {
       ...
       table.row.add([
            ...
            data.position,
            data.email,
            '<center><button data-toggle="tooltip" title="Update User Details" class="btn btn-success edit" value="'+data.user_id+'" id="btn_edit_'+data.user_id+'" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button><button data-toggle="tooltip" title="Send User to Archive" class="btn btn-danger archive"  value="'+data.user_id+'" id="btn_archive_'+data.user_id+'"><i class="fa fa-archive" aria-hidden="true"></i></button></center><h4><center> <span id="label_edit_'+data.user_id+'" class="label label-default" style="display:none">Updating <img src="images/ajax-loader.gif" style="height: 10px; width: 10px;" alt="DOST-IAS"> </span></h4>'
        ]).draw();
  ...
  $(document.body).on('click', '.edit', function(e) {
      ...
      var srow = $('#auditors_row');
      var _id = $('#updateAuditorsForm input[name="_id"]').val();

      if (_id != '0') { // another row is currently being edited
        showAvailableActions('edit_cancel', _id);
      }

      $('#updateAuditorsForm input[name="_id"]').val(id);
                  
      // Update actions column                
      showAvailableActions('edit', id);
  });

  $(document.body).on('click', '.update_auditorss', function(e) {
      $.ajaxSetup({
          headers: {
              'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
          }
      ...
      var srow = $('#auditors_row');
      var form = document.querySelector('#updateAuditorsForm');
      var formdata = new FormData(form)
      ...
      $.ajax({
          ...
          success : function(data){
            ...
            jQuery("#s_agency_" + id).fadeOut( 100 , function() {
                jQuery(this).html( data);
                 $('#s_agency_' + id ).text(u.agency);
            }).fadeIn( 1000 );
...
function showAvailableActions($event, $id) {
    ...
    $("#u_agency").easyAutocomplete(options);
